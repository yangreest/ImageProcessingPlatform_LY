		...
		if (m_vectorDataBuffer[0] == 0xff && m_vectorDataBuffer[1] == 0xfe)
		{
			...
			if (m_vectorDataBuffer[packLen - 2] == 0xfd && m_vectorDataBuffer[packLen - 1] == 0xfc)
			{
				...
				if (checkSum == checkSum2)
				{
					...
					case 0x03:
					{
						if (!m_vectorCmdData.empty() && m_function_XRaySendResult != nullptr)
						{
							m_function_XRaySendResult(m_vectorCmdData[0]);
						}
						break;
					}
					case 0x07:
					...
					{
						//进入OTA模式的反馈
						if (!m_vectorCmdData.empty() && m_vectorCmdData[0] > 0x00)
						{
							m_cOTAErrorCount = 0;
							m_cOTAStatus = 2;
...
void CWHSDControlBoardProtocol::RegisterXRaySendResult(const std::function<void(uint8_t)>& f)
{
...
std::vector<uint8_t> CWHSDControlBoardProtocol::SetEncoderZero()
{
	return GetCmdData(0x15, {0b1});
}

std::vector<uint8_t> CWHSDControlBoardProtocol::SetMotorControlMode(uint8_t mc)
{
	return GetCmdData(0x16, { mc });
}
