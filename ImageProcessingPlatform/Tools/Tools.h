...
namespace WHSD_Tools
{
	...
	bool SaveDataToFile(uint8_t* data, int len, const char* path);

	bool SaveDataToFile_Raw(uint8_t* data, int len, const std::string& fileType = ".sdraw");

	bool SaveDataToFile2_Raw(uint8_t* data, int len, const std::string& fileType = ".sdraw");

	bool SaveFileByGuid(uint8_t* data, int len, const std::string& guid, int workMode,
		const std::string& fileType = ".sdraw");
	...
	std::string CalculateDataMD5(const uint8_t* dat, int len);

	void CalculateOriginalCoordinates(double transformedX, double transformedY,
