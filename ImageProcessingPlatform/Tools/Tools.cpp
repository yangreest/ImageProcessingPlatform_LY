...
bool WHSD_Tools::SaveDataToFile(uint8_t* data, int len, const char* path)
{
	try
	{
...
bool WHSD_Tools::SaveDataToFile_Raw(uint8_t* data, int len, const std::string& fileType)
{
	try
	{
...
bool WHSD_Tools::SaveDataToFile2_Raw(uint8_t* data, int len, const std::string& fullPath)
{
	try
	{
		// 保存二进制文件
...
std::string WHSD_Tools::CalculateDataMD5(const uint8_t* dat, int len)
{
	// 1. 输入参数合法性校验（优先处理非法参数，避免后续无效操作）
	if (len < 0)
	{
		return "";
	}
	if (len > 0 && dat == nullptr)
	{
		return "";
	}

	// 初始化资源句柄（默认无效值，确保后续释放逻辑安全）
	HCRYPTPROV hCryptoProv = NULL;
	HCRYPTHASH hMD5Hash = NULL;
	unsigned char md5BinaryResult[16] = { 0 }; // MD5固定16字节二进制结果
	DWORD md5ResultLen = sizeof(md5BinaryResult);
	std::string md5HexResult; // 最终返回的十六进制字符串

	// 2. 获取加密服务提供程序（失败则直接释放已占资源并返回）
	if (!CryptAcquireContext(
		&hCryptoProv,
		NULL, // 不指定密钥容器，使用默认上下文
		MS_ENH_RSA_AES_PROV, // 现代加密服务提供程序，兼容MD5
		PROV_RSA_AES,
		CRYPT_VERIFYCONTEXT // 仅验证上下文，避免权限问题（无需创建密钥容器）
	))
	{
		// 此时仅hCryptoProv可能非空，直接释放后返回
		if (hCryptoProv != NULL)
		{
	...
	// 4. 写入二进制数据到哈希对象（仅当数据长度>0时处理）
	bool isHashDataSuccess = true;
	if (len > 0)
	{
		if (!CryptHashData(
			hMD5Hash,
			dat, // 输入数据指针
			static_cast<DWORD>(len), // 转换为DWORD适配CryptoAPI参数要求
			0
		))
		{
			isHashDataSuccess = false;
		}
	}

	// 5. 仅当数据写入成功时，获取MD5结果并转换为十六进制
	if (isHashDataSuccess)
	{
		if (CryptGetHashParam(
			hMD5Hash,
			HP_HASHVAL, // 参数类型：获取最终哈希值
			md5BinaryResult,
			&md5ResultLen,
			0
		))
		{
			// 二进制转32位小写十六进制（不足2位补0，确保格式统一）
			std::stringstream hexStream;
			hexStream << std::hex << std::setfill('0');
			for (DWORD i = 0; i < md5ResultLen; ++i)
			{
				hexStream << std::setw(2) << static_cast<unsigned int>(md5BinaryResult[i]);
			}
			md5HexResult = hexStream.str();
		}
		else
		{
			md5HexResult = "";
	...
	// 6. 释放所有已申请的资源（按“后创建先释放”顺序，避免依赖冲突）
	if (hMD5Hash != NULL)
	{
...
void WHSD_Tools::CalculateOriginalCoordinates(double transformedX, double transformedY,
	...
	double* originalX, double* originalY)
{
		...
		// 5.2 应用逆旋转（使用负角度）
		double rad = -rotationAngle * 3.1415926535898 / 180.0;
		double cosA = cos(rad);
		double sinA = sin(rad);
